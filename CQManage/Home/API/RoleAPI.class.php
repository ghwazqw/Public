...
class RoleAPI
{
	...
	function loadmatedata(){
		...
		$where['role_name']  = array('like',"%$this->_keyword%"); //成果名称
            ...
            $mmmap['_logic']='AND';
		//加载主表数据
		$this->_page_count=$count=M("qx_role_tb")->where($mmmap)->order($sort." ". $order)->count();
		$Page = new \Think\Page($count,$limit);

		$this->_page_bar=$Page->show(); //把分布内容赋值给变量
		$this->_main_data=$ret=M("qx_role_tb")->where($mmmap)->order($sort." ". $order)->LIMIT($offset.','.$limit)->select(); //主表数据取值完成
